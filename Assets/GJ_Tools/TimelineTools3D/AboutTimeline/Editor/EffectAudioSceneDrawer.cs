...
public static class EffectAudioSceneDrawer
{
    ...
    private static void DrawEffectPreview(SceneView sceneView)
    {
        ...
        Transform root = director.GetGenericBinding(track) as Transform;
        if (root == null)
        {
            return;
        }
        ...
        else if (tool == Tool.Rotate)
        {
            ...
            if (Quaternion.Angle(localRot, newLocalRot) > 0.01f)
            {
                ...
                asset.spawnEuler = newLocalRot.eulerAngles;
                EditorUtility.SetDirty(asset);
            }
        }
        ...
        TimelineSceneStyle.Tag($"特效预览:{asset.effectPrefab.name}", worldPos + Vector3.up * 0.3f, TimelineSceneStyle.Effect);
    }
}
