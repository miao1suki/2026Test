...
拼图的基准网格永远来自当前 `CubeMapWorkspaceDefinition`：

`
基准格宽 = FaceWidth / ColumnsPerFace
基准格高 = PieceHeight / RowsPerPiece
`

默认工作区是 12 × 4 世界单位、6 × 2 基准格，所以物品尺寸单位为 2 × 2。策划不应
在物品预制体上手填世界尺寸；物品的宽高只填写“占用几格”。

物品的占用尺寸仍以基准格记录，但实际摆放使用更细的放置网格。默认每个基准格
再细分 4 次，因此默认放置步长为 0.5 × 0.5 世界单位。可在 UI 的“当前网格”中修改
“放置网格细分”（1–16）。鼠标位置会让物品中心吸附到最近的合法放置步点，并自动
夹在面边界内；已有实例的实际 Transform 位置用于选择和占用检测，不会因细分调整
而漂移。

每个小拼图的四个面各自共享同一套放置网格，编辑面可在 UI 的“当前网格”区域切换：
