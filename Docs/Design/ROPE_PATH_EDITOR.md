...
Scene 窗口右侧统一的“路径机关工具”提供傻瓜式创建流程：

1. 一键创建网络和绳子；选中绳子后可直接从 A/B 端继续搭建；
2. 一键创建平台并自动绑定当前或最近的绳端；
3. 切换 Front/Right/Back/Left 投影方向并调整接续容差；
...
8. 通过“检查并修复当前场景”补齐白膜、平台碰撞体、父级网络并重建路径缓存。

面板关闭后可从 `Tools > 2026Test > 路径机关 > 显示傻瓜式 Scene 工具` 重新打开。完整策划流程见 [PATH_MECHANISM_TOOL.md](PATH_MECHANISM_TOOL.md)。
